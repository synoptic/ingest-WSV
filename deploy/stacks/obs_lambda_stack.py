...
class {INGEST_NAME}ObsLambda(Stack):

    def __init__(self, scope: Construct, id: str, lambda_env: dict, config: dict, **kwargs) -> None:
        super().__init__(scope, id, description=f"Lambda function to process {INGEST_NAME} Data", **kwargs)
        ...
        {INGEST_NAME}_lambda  = _lambda.DockerImageFunction(
                    scope=self,
                    id="{INGEST_NAME}ObsLambda",
                    # Function name on AWS
                    function_name="{INGEST_NAME}ObsLambda",
                    memory_size=256,
